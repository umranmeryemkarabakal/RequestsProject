"""
asyncio kütüphanesi
internet ile çalışırken mantıklıdır
coroutines durdurulup devam edilebilen sistemdir.
async: senkron olmayacak şekilde fonksiyon yazacağımızı belirtir
await: fonksiyon sonucunu beklemek için kulanılır
asyncio : io input output web geliştirmede kullanılır
"""
...
async def get_data_async_concurrently(urls):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = []
        for url in urls:
            tasks.append(asyncio.ensure_future(get_data(session, url, json_array)))#ensure yerine create kullanılanilir
        await asyncio.gather(*tasks) #*task for loopla tek tek konulmuş gibi olur
    et = time.time()
    elapsed_time = et - st
    print('Execution time:', elapsed_time, 'seconds')
    return json_array
